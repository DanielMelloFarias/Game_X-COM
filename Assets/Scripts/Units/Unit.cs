...
public class Unit : MonoBehaviour 
{
	...
	protected IEnumerator Move(Tile targetTile)
	{
		...
		DecreseMoviment(UnitManager.unitManager.selectedUnit, currentMovement);			
	...
	void DecreseMoviment (Unit movimentUnit, float remainingMovement)
    {
		
		movimentUnit.gameObject.GetComponentInChildren<Text>().text = ((int)remainingMovement).ToString();

		//Debug.Log("Texto: " + movimentUnit.gameObject.GetComponentInChildren<Text>().text);

		/*Debug.Log("Movimentos Restantes Antigo: " + movimentUnit.movement);
		Debug.Log("Movimentos Restantes Float: " + remainingMovement);
		Debug.Log("Movimentos Restantes Int: " + (int)remainingMovement);
		*/
	...
	IEnumerator AttackRoutine(Unit attackingUnit) //Performs various attack actions based on the current conditions
	{
		...
		attackingUnit.gameObject.GetComponentInChildren<Text>().text = "0";
